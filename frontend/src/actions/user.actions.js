...
const uploadAvatar = (data, userId) => {
    return (dispatch) => {
        ...
        fetch(`${process.env.HOST}/aws/upload`, {
            ...
            avatarLink = response;
            return fetch(`${process.env.HOST}/avatar`, {
                method: 'PUT',
...
const uploadGeolocation = (userId, lat, lng) => {
    return dispatch => {
        fetch(`${process.env.HOST}/geolocation/home`, {
            method: 'PUT',
            headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
...
const loadGeoCirclesList = (userId) => async (dispatch) => {
    try {
        let response = await fetch(`${process.env.HOST}/geolocation/circleslist/${userId}`, {
            method: 'GET',
            credentials: 'include',
        });
        if (response.ok) {
...
const loadGeolocation = (id) => async (dispatch) => {
    try {
        let response = await fetch(`${process.env.HOST}/geolocation/${id}`, {
            method: 'GET',
            credentials: 'include',
        });
        if (response.ok) {
...
const cancelFirstTimer = (userId) => async (dispatch) => {
    try {
        let response = await fetch(`${process.env.HOST}/firstTimer`, {
            method: 'PUT',
            credentials: 'include',
